import React, { Component } from "react";
import "bootstrap/dist/css/bootstrap.min.css";
import { BrowserRouter as Router, Switch, Route, Link } from "react-router-dom";

import Login from "./components/admin/login";
import Home from "./components/home/home";
import AboutUs from "./components/about/aboutus";
import ContactUs from "./components/contact/contactus";
import AddAdmin from "./components/admin/addAdmin/AddAdmin";
import IndexAdmin from "./components/admin/indexAdmin/IndexAdmin";
import TableRow from "./components/admin/indexAdmin/TableRow";
import UpdateAdmin from "./components/admin/updateAdmin/UpdateAdmin";
import AdminMenu from "./components/admin/adminMenu";
import AddCourse from "./components/course/addCourse/AddCourse";
import IndexCourse from "./components/course/indexCourse/IndexCourse";
//import TableRow from "./components/course/indexCourse/TableRow";
import UpdateCourse from "./components/course/updateCourse/UpdateCourse";
import AllStudenets from "./components/student/AllStudenets";
import AddInstructor from "./components/instructor/addInstructor/AddInstructor";

//instructor imports
import iMainPage from "./components/instructor/main";
import iNotificationPage from "./components/instructor/notification";
import iCreateExamPage from "./components/instructor/createExam";
import iEditExamPage from "./components/instructor/editExam";
import iEditViewExamPage from "./components/instructor/editViewExam";
import iMarkExamPage from "./components/instructor/markExam";
import iMarkViewExamPage from "./components/instructor/markViewExam";
import iCreateAssignmentPage from "./components/instructor/createAssignment";
import iEditAssignmentPage from "./components/instructor/editAssignment";
import iEditViewAssignmentPage from "./components/instructor/editViewAssignment";
import iMarkAssignmentPage from "./components/instructor/markAssignment";
import iMarkViewAssignmentPage from "./components/instructor/markViewAssignment";
//----------------------------------------

class App extends Component {
  render() {
    return (
      <Router>
        <div className="container">
          <nav className="navbar navbar-expand-lg navbar-dark bg-dark">
            <img
              src="data:image/jpeg;base64,"
              height="80"
              width="80"
            />
            &nbsp;
            <Link to={"/"} className="navbar-brand">
              <b>University Of Information Technology</b>
              <br />
              Guider of the IT future...
            </Link>
            <div
              className="collapse navbar-collapse"
              id="navbarSupportedContent"
            >
              <ul className="navbar-nav mr-auto">
                <li className="nav-item">
                  <Link to={"/home"} className="nav-link">
                    Home
                  </Link>
                </li>

                <li className="nav-item">
                  <Link to={"/Login"} className="nav-link">
                    Login
                  </Link>
                </li>

                <li className="nav-item">
                  <Link to={"/about"} className="nav-link">
                    About Us
                  </Link>
                </li>

                <li className="nav-item">
                  <Link to={"/contact"} className="nav-link">
                    Contact Us
                  </Link>
                </li>

                {/* <li className="nav-item">
                  <Link to={"/admin"} className="nav-link">
                    addAdmin
                  </Link>
                </li> */}
              </ul>
            </div>
          </nav>{" "}
          <br />
          <Switch>
            <Route path="/home" component={Home} />
            <Route path="/login" component={Login} />
            <Route path="/about" component={AboutUs} />
            <Route path="/contact" component={ContactUs} />
            <Route path="/admin" component={AddAdmin} />
            <Route path="/instructor" component={AddInstructor} />
            <Route path="/course" component={AddCourse} />

            {/* instructor route paths */}
            <Route path="/iMain" component={iMainPage} />
            <Route path="/iNotification" component={iNotificationPage} />
            <Route path="/iCreateExam" component={iCreateExamPage} />
            <Route path="/iEditExam/:id" component={iEditExamPage} />
            <Route path="/iEditViewExam" component={iEditViewExamPage} />
            <Route path="/iMarkExam" component={iMarkExamPage} />
            <Route path="/iMarkViewExam" component={iMarkViewExamPage} />
            <Route
              path="/iCreateAssignment"
              component={iCreateAssignmentPage}
            />
            <Route
              path="/iEditAssignment/:id"
              component={iEditAssignmentPage}
            />
            <Route
              path="/iEditViewAssignment"
              component={iEditViewAssignmentPage}
            />
            <Route path="/iMarkAssignment" component={iMarkAssignmentPage} />
            <Route
              path="/iMarkViewAssignment"
              component={iMarkViewAssignmentPage}
            />
            {/* ------------------------- */}
          </Switch>
        </div>
      </Router>
    );
  }
}

export default App;
